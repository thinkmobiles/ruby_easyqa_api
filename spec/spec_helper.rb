...
def action_stub(object_name, action_name, http_method, stubbing_url)
...
def action_all_stub(object_name, stubbing_url)
  stub_request(:get, "#{BASE_URL}#{stubbing_url}")
    .with(query: hash_including(auth_token: DATA['user']['attrs']['auth_token']))
    .to_return(
      body: DATA[object_name.to_s]['webmock']['all']['success']['response']['body'].to_json
    )

  stub_request(:get, "#{BASE_URL}#{stubbing_url}")
    .with(query: hash_excluding(auth_token: DATA['user']['attrs']['auth_token']))
    .to_return(
      body: DATA[object_name.to_s]['webmock']['all']['error']['response']['body'].to_json,
      status: DATA[object_name.to_s]['webmock']['all']['error']['response']['status']
    )
end
